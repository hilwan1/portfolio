    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-center px-4 pt-4 pb-3 md:px-6">
        <div className="flex w-full max-w-[720px] min-h-[4.2rem] items-center gap-4 rounded-full border border-neutral-800 bg-[#111111]/90 px-4 py-2 shadow-lg backdrop-blur-xl">
          <Link href="/" className="flex items-center justify-center w-11 h-11 shrink-0 rounded-full bg-[#1a1a1a] hover:bg-orange-500 transition-colors">
            <ArrowLeft className="w-5 h-5 text-white" />
          </Link>
          <div className="relative flex items-center justify-center w-[2.85rem] h-[2.85rem] shrink-0 rounded-full border border-orange-500/30 bg-orange-500/10">
      ...
      <div className="relative z-0 flex-1 flex flex-col overflow-y-auto px-4 py-6 scroll-smooth custom-scrollbar">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center w-full max-w-[620px] mx-auto border border-orange-500/20 rounded-[1.8rem] bg-gradient-to-b from-[#111] to-orange-950/20 p-8 md:p-12 text-center shadow-xl animate-fadeUp">
            <div className="flex items-center justify-center w-20 h-20 mb-5 rounded-full border border-orange-500/30 bg-orange-500/10 shadow-lg text-orange-500">
              <Bot className="w-10 h-10" />
            </div>
            <h2 className="text-3xl md:text-5xl font-bold m-0 mb-4 tracking-tight text-orange-50">Silakan Bertanya!</h2>
            <p className="max-w-[500px] text-orange-200/60 text-sm md:text-base mb-8 leading-relaxed">
            ...
            <div className="flex flex-wrap justify-center gap-3 w-full">
              <button 
                onClick={() => handleInputChange({ target: { value: 'Ceritakan tentang pengalaman kerja kamu dong!' } } as any)}
                className="flex items-center gap-2 border border-orange-500/30 rounded-full bg-orange-500/10 text-orange-500 px-4 py-2.5 text-xs font-mono font-semibold hover:bg-orange-500 hover:text-white transition-colors"
              >
                <Briefcase className="w-4 h-4" /> Pengalaman Kerja
              </button>
              <button 
                onClick={() => handleInputChange({ target: { value: 'Boleh lihat portfolio Web UI/UX kamu?' } } as any)}
                className="flex items-center gap-2 border border-orange-500/30 rounded-full bg-orange-500/10 text-orange-500 px-4 py-2.5 text-xs font-mono font-semibold hover:bg-orange-500 hover:text-white transition-colors"
              >
                <PenTool className="w-4 h-4" /> Portfolio Web UI/UX
              </button>
              <button 
                onClick={() => handleInputChange({ target: { value: 'Boleh lihat project desain logo yang pernah kamu buat?' } } as any)}
                className="flex items-center gap-2 border border-orange-500/30 rounded-full bg-orange-500/10 text-orange-500 px-4 py-2.5 text-xs font-mono font-semibold hover:bg-orange-500 hover:text-white transition-colors"
              >
                <Zap className="w-4 h-4" /> Seputar Logo
              </button>
            </div>
          </div>
        ) : (
          <div className="flex flex-col w-full max-w-[820px] mx-auto gap-4 pb-4">
            {messages.map((m: any) => (
              ...
              >
                {m.role !== 'user' && (
                  <div className="flex items-center justify-center w-8 h-8 shrink-0 rounded-full bg-orange-500/10 border border-orange-500/30 mb-1">
                ...
                <div 
                  className={`max-w-[85%] md:max-w-[78%] border rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                    m.role === 'user' 
                      ? 'bg-orange-500 text-white rounded-br-sm border-transparent' 
      ...
      <div className="relative z-10 shrink-0 border-t border-neutral-900 bg-[#050505]/95 backdrop-blur-xl pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] px-4 md:px-6">
        <form onSubmit={handleSubmit} className="relative flex w-full max-w-[820px] mx-auto">
          <input
            ref={inputRef}
            className="w-full border border-neutral-800 rounded-full bg-[#111111] text-white px-5 py-4 pr-16 text-sm focus:outline-none focus:border-orange-500 transition-colors placeholder:text-neutral-500"
            value={input}
          ...
          <button 
            type="submit" 
            className="absolute top-1.5 right-1.5 bottom-1.5 flex items-center justify-center w-11 rounded-full bg-orange-500 text-white hover:bg-orange-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" 
